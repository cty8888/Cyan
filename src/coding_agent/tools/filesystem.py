...
class WriteFileTool(Tool):
    ...
    def describe(self, args: dict[str, Any], policy: SecurityPolicy) -> tuple[str, str | None, str]:
        ...
        diff = _unified_diff(old_content, new_content, policy.display(target))
        return f"{action}文件 {policy.display(target)}", diff, "diff"
    ...
    def run(self, ctx: ToolContext, path: str, content: str) -> ToolResult:
        ...
        if existed and not ctx.session.has_read(target):
            raise ToolError(
                f"{ctx.policy.display(target)} 已存在，但本会话中还没有读取过它。"
        ...
        return ToolResult.success(
            f"已{action} {ctx.policy.display(target)}（{line_count} 行）",
            diff=_unified_diff(old_content, content, ctx.policy.display(target)),
        )
...
class EditFileTool(Tool):
    ...
    def describe(self, args: dict[str, Any], policy: SecurityPolicy) -> tuple[str, str | None, str]:
        ...
        diff = _unified_diff(original, updated, policy.display(target))
    ...
    def run(
        ...
        return ToolResult.success(
            ...
            diff=_unified_diff(original, updated, ctx.policy.display(target)),
        )
